...
/*   Updated: 2020/07/21 14:53:59 by awerebea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	f_check_screenshot_arg(char **argv, t_sdf *opts)
{
	...
	{
		if (!(opts->screenshot_arg = malloc(len + 1)))
			return (200);
		ft_strlcpy(opts->screenshot_arg, argv[2], len + 1);
		return (103);
	}
	else
		opts->screenshot = 1;
...
int			f_check_args(int argc, char **argv, t_sdf *opts)
{
	...
	{
		if (!(opts->map_file = malloc(len + 1)))
			return (200);
		ft_strlcpy(opts->map_file, argv[1], len + 1);
		return (102);
	}
	if (argc == 3)
	{
