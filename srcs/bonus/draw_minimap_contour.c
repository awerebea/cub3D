...
/*   Updated: 2020/08/29 23:33:42 by awerebea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		f_draw_minimap_contour_assist_1(t_mlx *mlx, int x, int y)
{
	...
	if ((mlx->map.x < mlx->map.map_width - 1 && mlx->map.sq_x == \
		...
		pixel_to_put = 1;
			...
			mlx->opts->map_array[mlx->map.y][mlx->map.x] != ' ')
		pixel_to_put = 1;
	else if (((mlx->map.x == 0 && mlx->map.sq_x == 0) || (mlx->map.y == 0 && \
			mlx->map.sq_y == 0)) && \
			mlx->opts->map_array[mlx->map.y][mlx->map.x] != ' ')
		pixel_to_put = 1;
	if (pixel_to_put)
...
static int		f_draw_minimap_contour_assist_2(t_mlx *mlx, int x, int y)
{
	...
	pixel_to_put = 0;
	if (((mlx->map.x > 0 && mlx->map.sq_x == 0 && \
			mlx->opts->map_array[mlx->map.y][mlx->map.x - 1] == ' ') || \
			(mlx->map.y > 0 && mlx->map.sq_y == 0 && \
			mlx->opts->map_array[mlx->map.y - 1][mlx->map.x] == ' ')) && \
			(mlx->opts->map_array[mlx->map.y][mlx->map.x] != ' '))
		pixel_to_put = 1;
	else if ((x == mlx->map.edge_shift || y == mlx->map.edge_shift) && \
			mlx->opts->map_array[mlx->map.y][mlx->map.x] != ' ')
		pixel_to_put = 2;
	...
	if (pixel_to_put == 1)
		my_mlx_pixel_put(&mlx->img, x, y, 0xFFFFFF);
	else if (pixel_to_put == 2)
		my_mlx_pixel_put(&mlx->img, x, y, 0xFF9999);
	return (pixel_to_put);
}
