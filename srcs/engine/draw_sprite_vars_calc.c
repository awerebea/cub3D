...
/*   Updated: 2020/09/04 10:52:29 by awerebea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	f_sprite_vars_calc_start(t_mlx *mlx, t_sp *sp, t_sp_vars *sp_vars)
{
	...
	sp_vars->vert_offset = (int)(mlx->y_win_size * 0.2 / sp_vars->transform_y);
}

void	f_sprite_vars_calc_finish(t_mlx *mlx, t_sp_vars *sp_vars)
{
	if ((sp_vars->height = (int)(mlx->y_win_size / sp_vars->transform_y)) < 0)
		sp_vars->height *= -1;
	...
	if ((sp_vars->width = (int)(mlx->y_win_size / sp_vars->transform_y)) < 0)
		sp_vars->width *= -1;
